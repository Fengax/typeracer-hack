...
has_link = input("Do you have a link? Enter (No) if you don't: ")
...
if has_link == "No":
    ...
    browser.get(has_link)
wrong_keys = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u",
              "v", "w", "x", "y", "z", " ", "'", ",", ".", ";", ":"]
...
while True:
    try:
        login_button = browser.find_element_by_css_selector(
            "#tstats > table > tbody > tr.datarow > td:nth-child(1) > table > tbody > tr > td:nth-child(1) > a")
        login_button.click()
        break
    except:
        pass
...
race_button = browser.find_element_by_css_selector(
    "#dUI > table > tbody > tr:nth-child(2) > td:nth-child(2) > div > div.mainViewport > div > table > tbody > tr:nth-child(4) > td > table > tbody > tr > td:nth-child(2) > table > tbody > tr:nth-child(1) > td > a")
race_button.click()
...
while True:
    time.sleep(10)
    join_race_button = browser.find_element_by_css_selector("#gwt-uid-18 > table > tbody > tr:nth-child(3) > td > table > tbody > tr > td:nth-child(2) > a")
        ...
        if rand_wrong_key == 5:
            ...
            try:
                for x in range(0, rand_wrong_key_iter):
                    input_box.send_keys(text[i + x + 1])
                    time.sleep(random.randint(10, 50) * delay)
